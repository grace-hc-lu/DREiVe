...
use Cwd;
@ISA = ('Exporter');
@EXPORT = ('chrom_alignment');

sub chrom_alignment{
my $ref = shift;
my @chrom_species = @_;

foreach $species (@chrom_species){
print "$species\n";
  ...
  while(<NET>){
      print "$species:$chrom:$_";
    $i++;
    if (/^net (.*) \d+/){
      $chrom = $1;
      %starts = (); 
      %gene_starts = ();
      $annotation = "annotations/hg19/$chrom.annotation"||print "Can not open annotation file\n";
      open (REFSEQ, "$annotation");
      while (<REFSEQ>){
        $gene_starts{$1}{$2}++ if /^\S+\tNM_\d+\t$chrom\t.\t(\d+)\t(\d+)\t/;
      }
      ...
      if ($wsf == 1){
        ...
        GENE:foreach $gene_start (sort {$a <=> $b} keys %gene_starts){
	  last GENE if $gene_start > $end;
          foreach $gene_end (keys %{$gene_starts{$gene_start}}){
            if ($start <= $gene_start && $gene_end <= $end){
              $fills{$wsf}{$chrom}{$start} = "$2\n";
              $starts{$wsf} = $start;
              last GENE;
	    }
      ...
      if ($wsf == $wsg + 1 && $wsf > 1 && $i == $igap + 1 && $fill_gap{$wsf-1}){
	$fill_gap{$wsf-1} = 0;
