...
vector<_motif> print_motifs(3000);
vector<int> print_posbeg(3000, 0);
vector<int> print_posend(3000, 0);
vector<_cluster_map> all_motifs(3000);
...
int main( int argc, char * argv[] )
{
  ...
  if(argc > 1) {
    ...
    if(input.bad()) {
      cout << "Input file contains formatting errors" << endl;
    }
    if(input.good()) {
...
//      _cluster_map *all_motifs[600] = new _cluster_map[600];
...
void sort_motif_cluster(_cluster_map* cluster_motif, _print_pos* already_printed, _print_pos* final_printed) {
 ...
 //
  ...
  int true_pos3 = 0;
  int last_seq = -1;
  vector<int> v8(3000, -1);
  vector<int> v7(3000, 0);
  vector<vector<int> > v9(numSeq, v8);
  vector<vector<int> > v10(numSeq, v8);
  vector<vector<int> > v11(numSeq, v7);
  vector<int> how_many(numSeq, -1);
...
//      }
  ...
  vector<int> v5(2, -1);
  vector<vector<int> > v12(numSeq, v5);
...
//  cout << " before print...print_active is: " << print_active << endl;
  ...
  _cluster_map new_motifs;
  bool do_we_print = true;
  if (no_written >= 2) {
    vector<int> v4(2, -1);
    vector<vector<int> > v14(numSeq, v4);
    print_active = true;
    for (_cluster_map::iterator it = valid_motifs.begin(); it != valid_motifs.end(); it++) {
      ...
      for (int j = 0; j < locs.size(); j++) {
        cluster_location cl1;
        cl1.seqId = locs[j].seqId;
        cl1.x0 = locs[j].x0;
        cl1.x1 = locs[j].x1;
        new_motifs[motifId].push_back(cl1);
  ...
  if ((print_active) && (do_we_print)) {
    ...
    for (_cluster_map::iterator it = new_motifs.begin(); it != new_motifs.end(); it++) {
      const int motifId = it->first;
      vector<cluster_location> locs = new_motifs[motifId];
      for (int j = 0; j < locs.size(); j++) {
        cluster_location cl1;
        cl1.seqId = locs[j].seqId;
        cl1.x0 = locs[j].x0;
        cl1.x1 = locs[j].x1;
        all_motifs[this_number][motifId].push_back(cl1);
      }
    }
    this_number++;
  }
...
//              all_match = true;
    ...
    if (print_active) {
      ...
      if(length >= min_length) {
        ...
        _full_map seq_print;
        for (_cluster_map::iterator it = all_motifs[j].begin(); it != all_motifs[j].end(); it++) {
          const int motifId = it->first;
          vector<cluster_location> locs = all_motifs[j][motifId];
          for (int l = 0; l < locs.size(); l++) {
            motif_location m1;
            m1.motifId = motifId;
            m1.offset = (*(*locationArray)[locs[l].seqId])[locs[l].x0].offset;
            seq_print[locs[l].seqId].push_back(m1);
          }
        }
      
        bool first_mot = true;
        for (_full_map::iterator it = seq_print.begin(); it != seq_print.end(); it++) {
          const int seqId = it->first;
          vector<motif_location> seqlocs = seq_print[seqId];
          first_mot = true;
          for (int m = 0; m < seqlocs.size(); m++) {
            if (first_mot) {
              out << ">" << seqId << endl;
              first_mot = false;
            }
            motif_location ml1;
            ml1.motifId = seqlocs[m].motifId;
            ml1.offset = seqlocs[m].offset;
            out << "(" << ml1.motifId << "," << ml1.offset << ")";
          }
          out << endl;
        }
        out << "*******************************************************************************************************************************" << endl;
...
//  cout << "in find motifs loop" << endl;
  _cluster_map motif_cluster[999];
  _cluster_map motif_cluster1[999];
...
//                  break;
        ...
        if (last_left < 0) {
          all_done_left = true;
//          cout << "setting all_done_left" << endl;
...
//              cout << " gets to b , all_done_right is: " << all_done_right << endl;
          // Else add the motif to the map
...
//          cout << "gets to 2...last right is: "<< last_right << " locations.size is: " << locations.size() << endl;
          
          if (last_right >= locations.size()-1) {
            all_done_right = true;
          }

        }
//          cout << "gets to 3...last right is: "<< last_right << " locations.size is: " << locations.size() << "all_done_right is: " << all_done_right << endl;
//          cout << "gets to 3...all_done_right is: " << all_done_right << endl;
//          cout << "gets to break in rhs" << endl;
          if (all_done_right) {
//            cout << " gets to all done right" << endl;
            break;
//            all_done_left = true;
          }
...
//  cout << "at end of  find motifs loop" << endl;
